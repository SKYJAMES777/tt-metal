...
void kernel_main() {
    ...
    // Compile args: [0]=cb_id_out, [1]=cb_id_meta, [2]=tile_height, [3..]=cache accessor, then
    // metadata accessor. tile_height divides kv tokens into the page-row unit (TILE_HEIGHT for TILE,
    // 1 for ROW_MAJOR), so the same kernel handles both layouts (fp8_e4m3 is ROW_MAJOR-only).
    ...
    const uint32_t kv_actual_global_t = meta[1] / tile_height;
    cb_meta.push_back(1);
