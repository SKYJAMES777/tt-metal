    ...
    for (uint32_t b = 0; b < batch; ++b) {
        ...
        for (uint32_t g = 0; g < group; ++g) {
            ...
            for (uint32_t out_block_index = 0; out_block_index < num_out_blocks_padded; out_block_index++) {
...
#ifdef UNTILIZE_OUT
                // Untilize this block (block_w tiles wide, out_block_h_normal tile-rows tall) from
                ...
                // mirror of the on-core tilize on the input side). On a ragged final out-block we
                // untilize the full normal block; only the out_block_h_actual valid rows are scattered
                // to DRAM by the writer, so the padded tail rows are produced but never written.
