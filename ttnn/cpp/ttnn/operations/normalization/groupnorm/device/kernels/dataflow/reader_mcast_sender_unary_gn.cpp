...
#ifdef TILIZE_IN
                        // ROW_MAJOR interleaved input: the DRAM buffer is laid out row-major, so a
                        // "page" is one row of the full channel width. Gather the rows for this
                        // block into cb_in0 in row-major order across the block width (tile_height
                        // rows per tile-row, block_w*tile_width datums per row) so the compute
                        // kernel can tilize the block on-core (mirrors the sharded ROW_MAJOR path).
                        // The tile id the TILE-layout reader would use decomposes into a (tile_row,
                        // tile_col) pair; each tile maps to tile_height consecutive row pages at a
                        // fixed column byte offset.
