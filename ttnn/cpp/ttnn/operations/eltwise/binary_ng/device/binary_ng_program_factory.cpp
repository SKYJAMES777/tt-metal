...
tt::tt_metal::ProgramDescriptor BinaryNgDeviceOperation::ProgramFactory::create_descriptor(
    ...
    // as_defines() keys the SFPU init off the input dtype, but quant/requant rounding depends on the
    // *output* dtype: for uint8 output we need fp32->uint8 rounding (full [0,255] range) instead
    // of the default fp32->int8 ([-128,127]). The per-tile quant_tile/requant_tile op is unchanged;
    // the packer narrows the int32 SFPU result to uint8. Dequant output is float, so it never needs
    // this override.
