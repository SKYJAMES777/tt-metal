...
private:
    ...
    static constexpr size_t kMaxConsumerBatchPerDevice =
        1u << 15;  // max batch size per device; total batch = this x num_devices
    static constexpr size_t kMaxConsumerBatchCap = 1u << 20;  // hard cap (corresponds to 32 devices)
    static constexpr size_t kRingHeadroomBatches = 4;
    static constexpr size_t kMaxRingCapacity = 1u << 22;

    size_t max_consumer_batch_records_ = 0;
    std::optional<CallbackRecordRing> ring_;
