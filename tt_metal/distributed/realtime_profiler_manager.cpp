...
void RealtimeProfilerManager::publish_pages(
    ...
    for (uint32_t page = 0; page < num_pages; ++page) {
        ...
        records.emplace_back(
            ...
            data_collector_->GetKernelSourcesForRuntimeId(static_cast<uint16_t>(rp[2])));
    }
    if (records.empty()) {
        return;
    }
...
RealtimeProfilerManager::RealtimeProfilerManager(const std::shared_ptr<MeshDevice>& mesh_device) :
    ...
    max_consumer_batch_records_ = std::min(kMaxConsumerBatchCap, kMaxConsumerBatchPerDevice * devices_.size());
    ring_.emplace(std::min(kMaxRingCapacity, max_consumer_batch_records_ * kRingHeadroomBatches));
...
uint32_t RealtimeProfilerManager::drain_device_pages(
    ...
    const uint32_t observed_max = peak_fifo_pages_.load(std::memory_order_relaxed);
    const bool fifo_reached_capacity = available >= RealtimeProfilerRuntimeSizes::fifo_pages &&
                                       observed_max < RealtimeProfilerRuntimeSizes::fifo_pages;
    if (available > observed_max) {
...
uint64_t RealtimeProfilerManager::run_receiver_loop() {
    ...
    constexpr auto kReceiverIdlePollBackoff = std::chrono::microseconds(100);
    ...
    while (!stop_.load(std::memory_order_acquire)) {
        ...
        if (num_pages == 0) {
            std::this_thread::sleep_for(kReceiverIdlePollBackoff);
        }
    }
...
void RealtimeProfilerManager::run_consumer(Consumer& consumer) {
    std::vector<tt::ProgramRealtimeRecord> records(max_consumer_batch_records_);
