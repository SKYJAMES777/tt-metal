 ...
 * Readers track their positions separately, so they fall behind and recover independently.
 *
...
public:
    ...
    private:
        ...
        void publish_impl(std::span<U> items) {
            ...
            const size_t skip = n > view_.capacity ? n - view_.capacity : 0;
            for (size_t k = skip; k < n; k++) {
                if constexpr (std::is_const_v<U>) {
                    view_.slot_at(head + k).store(items[k]);
                } else {
                    view_.slot_at(head + k).store(std::move(items[k]));
                }
            }
    ...
    public:
         ...
         *
         * Consume only the returned span. It is a view into @p out, but may shorter and offset
         ...
         * @return The items read, oldest first (a sub-span of @p out); empty if nothing is ready: the
         *         reader is caught up, or just dropped an overwritten batch (call again).
         */
        [[nodiscard]] std::span<T> read_batch(std::span<T> out) noexcept(kLoadNoexcept) {
            if (out.empty()) {
                return out;
            }
            // the furthest the cursor may fall behind claim; beyond it the oldest records would (likely) be overwritten
            // before we finish copying them, so we drop them now and spend the copies on records we can actually keep
            ...
            // running max of how far the writer advances while we copy a batch, plus 25% headroom;
            // decays slowly so it tracks recent advances rather than pinning to the highest ever seen
            ...
            const uint64_t head = shared_state_->head.load(std::memory_order_acquire);
            if (cursor_ >= head) {
                return out.first(0);
            }
            const uint64_t claim_before = shared_state_->claim.load(std::memory_order_relaxed);
            const uint64_t cap = view_.capacity;
            const uint64_t want = std::min<uint64_t>(out.size(), cap);
            const uint64_t lag_limit = max_lag(want, cap);
            ...
            for (size_t k = 0; k < n; k++) {
                view_.slot_at(start + k).load(out[k]);
            }
        ...
        /** @brief Reads one item into @p out; returns false when caught up. */
        ...
        void wait() const noexcept {
            const auto observed_wake_token = shared_state_->wake_token.load(std::memory_order_acquire);
            if (cursor_ < shared_state_->head.load(std::memory_order_acquire)) {
                return;  // necessary in case wake_readers() was called for the last publish before we read the token
            }
            for (uint32_t spin = 0; spin < kWaitSpinIterations; ++spin) {
                if (shared_state_->wake_token.load(std::memory_order_acquire) != observed_wake_token) {
                    return;
                }
                ttsl::pause();
            }
            shared_state_->wake_token.wait(observed_wake_token, std::memory_order_acquire);
        }
