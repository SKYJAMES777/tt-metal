...
TEST(RealtimeProfilerSanity, FiveProgramsBackToBack) {
    ...
    ProgramRealtimeProfilerCallbackHandle handle_a = RegisterProgramRealtimeProfilerCallback(
        [&records_mu, &records_a, &dropped_a](const ProgramRealtimeRecordBatch& batch) {
    ...
    ProgramRealtimeProfilerCallbackHandle handle_b = RegisterProgramRealtimeProfilerCallback(
        [&records_mu, &records_b, &dropped_b](const ProgramRealtimeRecordBatch& batch) {
    ...
    std::vector<ProgramRealtimeRecord> collected_b;
    {
        std::lock_guard<std::mutex> lk(records_mu);
        collected = std::move(records_a);
        collected_b = std::move(records_b);
    }
